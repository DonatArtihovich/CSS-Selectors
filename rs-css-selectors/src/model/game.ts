import { levelsArr, headerText, winTableLabelText, winTableSublabelText } from '../utils/const'
import { type ITitleObject, type ILevel } from '../utils/Types'
import { ViewerStartInners } from '../utils/enums'
...
function renderLevel (level: ILevel): void {
  ...
  if (level === undefined || level.createItemElements === undefined) {
...
function changeHeader (header: string): void {
  ...
  if (headerElement == null) {
    throw new Error('Unexpected null instead of header!')
  }
...
function changeHTMLViewer (level: ILevel): void {
  ...
  if (viewer == null) {
    throw new Error('Unexpected null instead of HTML Viewer!')
  }
...
export function changeLevelIndex (index: number): void {
  ...
  if (levelsArr.length <= currentLevelIndex) {
    return
  }
  ...
  if ((activeLevel == null) || (newActiveLevel == null)) {
    throw new Error('Unexpected null against of active level!')
  }
...
function addHighlightListeners (nodesArr: NodeListOf<HTMLElement>): void {
  nodesArr.forEach((elem: HTMLElement) => {
    elem.addEventListener('mouseover', () => {
      if (elem.dataset.highlight === undefined) {
        throw new Error('Unexpected undefined')
      }
...
function winLevel (): void {
  ...
  if (currentLevel.classList.contains('level-item_completed')) {
    currentLevel.classList.remove('level-item_completed')
  }

  if (currentLevel.classList.contains('level-item_help')) {
    currentLevel.classList.remove('level-item_help')
  }
...
function saveProgress (): void {
  ...
  const levelStatusArr: number[] = [...levelsArr].map((level) => {
    if (level.classList.contains('level-item_completed')) {
      return 1
    } else if (level.classList.contains('level-item_help')) {
      return 2
    }
    return 0
  }
